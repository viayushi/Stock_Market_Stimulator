...
const Login = () => {
  ...
  const { login, refreshPortfolio } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.user && response.user.firstLogin === true) {
        setShowConfetti(true);
        setShowPopup(true);
        // Refresh portfolio to ensure 5cr is shown
      ...
      {showPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl p-8 text-center">
            <h2 className="text-2xl font-bold mb-2 text-green-600">Welcome to StockSim!</h2>
            <p className="text-gray-700 mb-4">Your account has been unlocked.<br/>You have been awarded <span className='font-bold text-blue-600'>₹5,00,00,000</span> virtual balance!</p>
          </div>
        </div>
      )}
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-2xl">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">Sign in to continue to StockSim</p>
          </div>
